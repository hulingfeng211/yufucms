...
function writable($var){
	$writeable = FALSE;
	$var = YUFUCMS_ROOT.$var;
	if(!is_dir($var)){
		@mkdir($var, 0777);
	}
	if(is_dir($var)){
		$var .= '/temp.txt';
		if(($fp = @fopen($var, 'w')) && (fwrite($fp, 'YUFUCMS'))){
			fclose($fp);
			@unlink($var);
			$writeable = TRUE;
		}
...
function clear(){
	$file['cache'] = array(YUFUCMS_ROOT."Admin/Runtime/Cache",YUFUCMS_ROOT."YUFUCMSApp/Runtime/Cache");
	$file['data'] = array(YUFUCMS_ROOT."Admin/Runtime/Data/_fields",YUFUCMS_ROOT."YUFUCMSApp/Runtime/Data/_fields");
	$file['temp'] = array(YUFUCMS_ROOT."Admin/Runtime/Temp",YUFUCMS_ROOT."YUFUCMSApp/Runtime/Temp");
	$file['logs'] = array(YUFUCMS_ROOT."Admin/Runtime/Logs",YUFUCMS_ROOT."YUFUCMSApp/Runtime/Logs");
	$file['runtime'] = array(YUFUCMS_ROOT."Admin/Runtime/~runtime.php",YUFUCMS_ROOT."YUFUCMSApp/Runtime/~runtime.php");
	$file['app'] = array(YUFUCMS_ROOT.YUFUCMS_ROOT."Admin/Runtime/~app.php",YUFUCMS_ROOT."YUFUCMSApp/Runtime/~app.php");
